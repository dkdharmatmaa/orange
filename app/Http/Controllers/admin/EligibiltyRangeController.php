...
class EligibiltyRangeController extends Controller
{
    ...
    public function print_eligibilty($id){
        $main_data=Eligibilty::where('id',$id)->with('plans','branch','association')->select('first_name','last_name','no_of_people','email_id','street_address','address','association_id','branch_name','branch_id','city','state','zip_code','comment','plan_id','created_at')->first()->toArray();
