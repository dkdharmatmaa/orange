      ...
      <!-- for filter table end -->
          <div class="text-center bg-white px-5 py-10 mt-5 rounded">
            <b-table :bordered="true" responsive :items="items" :fields="fields" style="white-space:nowrap" primary-key="id" :filter="filter" :current-page="currentPage" :per-page="perPage"  @filtered="onFiltered">
              ...
              <router-link :to="'/admin/installment/'+data.item.subscription_refid" class="h5">{{data.item.subscription_refid}}</router-link>
              </template>
...
export default {
  ...
  methods: {
    ...
    get_report(){
      ...
      this.items=[];
      this.totalRows=0
      ApiService.post("/admin/all-installment",this.condition_arr)
        .then(({ data }) => {
          this.items=data;
