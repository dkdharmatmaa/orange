<template>
  <div class="mt-5">
    <div class="d-flex ml-2 justify-content-between">
          <h2>Products</h2>&nbsp;&nbsp;&nbsp;
          <button class="btn create_btn font-weight-bold"><router-link class="text-white" to="/admin/new-product">Create Plan</router-link></button>
      </div>
      ...
      <!-- for filter table end -->
          <div class="text-center bg-white px-5 py-10 mt-5 rounded">
            <b-table :bordered="true" responsive :items="items" :fields="fields" style="white-space:nowrap" primary-key="id" :filter="filter" :current-page="currentPage" :per-page="perPage"  @filtered="onFiltered">
              ...
              <button class="btn-trans-b-0"><router-link :to="'/admin/new-product/'+data.item['id']" class="text-white"><i class="fas fa-edit text-success"></i></router-link></button>
              </template>
              <template #cell(delete)="data">
              <button class="text-white btn-trans-b-0" @click="delete_product(data.item['id'])"><i class="far fa-trash-alt text-danger"></i></button>
              </template>
           </b-table>
          <!-- for pagination -->
...
export default {
  ...
  methods: {
    ...
    get_product(){
      ApiService.get(`/admin/all-product`)
        .then(({ data }) => {
          this.items=data;
