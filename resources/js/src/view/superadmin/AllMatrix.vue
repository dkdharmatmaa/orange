    ...
    <div class="d-flex ml-2 justify-content-between">
          ...
          <div class="d-flex">
              <b-form-group class="ml-3">
                <b-form-select id="input-3" v-model="association_id" :options="options_association" @change="getBranches(); get_plans_clear()" class="input-box text-seconday" required></b-form-select>
              </b-form-group>
              <b-form-group class="ml-3">
...
export default {
  ...
  methods: {
    ...
    delete_matrix(minmum_range){
      ...
      if(proceed){
       ApiService.delete(`/superadmin/delete-matrix/${minmum_range}/${this.association_id}/${this.branch_id}`)
        .then(({ data }) => {
          this.get_plans();
        })
      }
