  ...
  <div class="mt-10 p-5">
    ...
    >
      ...
      <div class="bg-white p-10 rounded">
        ...
        <b-form-group>
          <b-form-select id="input-3" v-model="user_form.association_id" :options="options_association" @change="getBranches()" class="ml-1 input-box text-seconday" required></b-form-select>
...
export default {
  ...
  methods: {
    onSubmit(evt) {
      evt.preventDefault();
      if(this.action=='Add'){
        ApiService.post(`/superadmin/create-user`, this.user_form)
        .then(({ data }) => {
        ...
        ApiService.put(`/superadmin/edit-user/${this.user_form.id}`, this.user_form)
        .then(({ data }) => {
    ...
    getBranches(){
        if(this.user_form.association_id){
            ApiService.get(`/superadmin/all-branch/0/${this.user_form.association_id}`)
            .then(({ data }) => {
                let branch_option=[];
